    ...
    for epoch in range(epochs):
        ...
        for i, batch in progress:
            ...
            for j in range(y.shape[0]):
                y_flat = y[j].flatten().cpu().detach().numpy()
                output_flat = output_binary[j][0].flatten()
                # Check for non-zero union
                if np.sum(y_flat) > 0 or np.sum(output_flat) > 0:
                    z = jaccard_score(y_flat, output_flat)
                    train_ious.append(z)
        ...
        for j, batch in progress:
            ...
            for k in range(y.shape[0]):
                # Flatten tensors for Jaccard score calculation
                y_true_flat = y[k].flatten().cpu().detach().numpy()
                output_binary_flat = output_binary[k][0].flatten()

                # Calculate Jaccard score only if both arrays contain non-zero elements
                if np.sum(output_binary[k][0]) != 0 and np.sum(y[k].cpu().detach().numpy()) != 0:
                    z = jaccard_score(y_true_flat, output_binary_flat)
                    val_ious.append(z)
